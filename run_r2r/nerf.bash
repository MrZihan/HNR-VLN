...
flag1="--exp_name release_r2r
	  ...
	  IL.ckpt_to_load pretrained/NeRF_p16_8x8.pth
...
flag2=" --exp_name release_r2r
      ...
      NUM_ENVIRONMENTS 1
      TASK_CONFIG.SIMULATOR.HABITAT_SIM_V0.ALLOW_SLIDING True
      EVAL.CKPT_PATH_DIR data/logs/checkpoints/release_r2r/ckpt.iter10000.pth
...
case $mode in 
      train)
      echo "###### train mode ######"
      CUDA_VISIBLE_DEVICES='6,7' python -m torch.distributed.launch --nproc_per_node=2 --master_port $2 run_nerf.py $flag1
      ;;
      eval)
      echo "###### eval mode ######"
      CUDA_VISIBLE_DEVICES='7' python -m torch.distributed.launch --nproc_per_node=1 --master_port $2 run_nerf.py $flag2
      ;;
      infer)
      echo "###### infer mode ######"
